...
extern uint8_t ce_pin; /**< "Chip Enable" pin, activates the RX or TX role */
extern uint8_t csn_pin; /**< SPI Chip select */
  
extern bool p_variant; /* False for RF24L01 and true for RF24L01P */
extern uint8_t payload_size; /**< Fixed size of payloads */
extern bool dynamic_payloads_enabled; /**< Whether dynamic payloads are enabled. */
extern uint8_t pipe0_reading_address[5]; /**< Last address set on pipe 0 for reading. */
extern uint8_t addr_width;
     /**
...
extern     bool rf24_failureDetected; 
#endif
...
void rf24_openWritingPipe_v(const uint8_t *address);
...
void rf24_openReadingPipe_v(uint8_t number, const uint8_t *address);
 ...
 */
bool rf24_write_noack( const void* buf, uint8_t len, const bool multicast );
 ...
 */
bool rf24_writeFast_noack( const void* buf, uint8_t len, const bool multicast );
 ...
 * @param enable Whether to enable (true) or disable (false) auto-acks
 */
void rf24_setAutoAck_on_pipe( uint8_t pipe, bool enable ) ;
...
#ifndef rf24_MINIMAL_NO_FULL_API
/**
 ...
 * @return Current value of status register
 */
uint8_t rf24_read_register_v(uint8_t reg, uint8_t* buf, uint8_t len);
 ...
 * @return Current value of status register
 */
uint8_t rf24_write_register_v(uint8_t reg, const uint8_t* buf, uint8_t len);
